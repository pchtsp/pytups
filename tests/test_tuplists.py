...
class TupTest(unittest.TestCase):
    ...
    def test_filter(self):
        result = [('a', 'c'), ('a', 'c'), ('a', 'c'),
                  ('r', 'c'), ('r', 'c'), ('r', 'c')]
        self.assertListEqual(result, self.prop1.filter([0, 2]))

    def test_filter2(self):
        result = [1, 2, 3, 1, 2, 3]
        self.assertListEqual(result, self.prop1.filter(3))

    def test_filter_order(self):
        result = [(1, 'a'), (2, 'a'), (3, 'a'), (1, 'r'), (2, 'r'), (3, 'r')]
        self.assertListEqual(result, self.prop1.filter([3, 0]))

    def test_filter_list_f(self):
        result = [('a', 'b', 'c', 1), ('a', 'b', 'c', 2), ('a', 'b', 'c', 3)]
        self.assertListEqual(result, self.prop1.filter_list_f(lambda x: x[0] <= 'a'))
